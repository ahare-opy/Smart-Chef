...
import { removeZero, removeFromInventory } from '../../utils/recipe';

import axios from 'axios';

function MyRecipeModal({ recipe, user }) {
  ...
  /*const [id, setId] = useState(recipe._id);
    ...
    const [time, setTime] = useState(recipe.cook_time_min);
    const [diet, setDiet] = useState(recipe.diet);
    const [serving, setServing] = useState(recipe.servings);
    const [pic, setPic] = useState(recipe.picture_link);
  ...
  useEffect(() => {
    (async () => {
      ...
      let newArray = JSON.parse(JSON.stringify(recipe));

      delete newArray._id;
      delete newArray.Name;
      delete newArray.cook_time_min;
      delete newArray.diet;
      delete newArray.servings;
      delete newArray.picture_link;
      delete newArray.type;
      delete newArray.link;

      let rrecipe = removeZero(newArray);

      setUserInventory(JSON.parse(JSON.stringify(rrecipe)));
  ...
  const handleCook = (e) => {
    ...
    removeFromInventory(userInventory, user);
  };
